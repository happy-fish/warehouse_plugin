...
def get_goods_types():
    ...
    types = mdb_web.dbs["plug_warehouse_type"].find({"user_id": current_user.str_id})
...
def add_goods_types():
    ...
    if mdb_web.dbs["plug_warehouse_type"].find_one({"name":name, "user_id":user_id}):
        ...
        mdb_web.dbs["plug_warehouse_type"].insert(up_data)
...
def update_goods_types():
    ...
    if mdb_web.dbs["plug_warehouse_type"].find_one({"name":name, "user_id":user_id,
        ...
        mdb_web.dbs["plug_warehouse_type"].update_one({"_id":ObjectId(id), "user_id":current_user.str_id},
                                                   {"$set":{"name":name}})
...
def del_goods_types():
    ...
    faied_cnt = 0
    for id in ids:
        if mdb_web.dbs["plug_warehouse_category"].find_one({"type_id":id, "user_id":user_id}):
            faied_cnt += 1
            continue
        elif mdb_web.dbs["plug_warehouse_business"].find_one({"type_id":id, "user_id":user_id}):
            faied_cnt += 1
            continue
        obj_ids.append(ObjectId(id))

    r = mdb_web.dbs["plug_warehouse_type"].delete_many({"_id": {"$in": obj_ids}, "user_id":user_id})
    if r.deleted_count:
